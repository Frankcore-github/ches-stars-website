...
<script>
    export default {
      ...
      methods: {
        ...
        getComments(){
            axios.get('/api/commentassignment/'+this.id).then(({data}) => (this.comment = data.object1, this.comment_count = data.object2, this.user = data.user))
        },
      ...
      mounted() {
        this.getComments()
        Echo.channel('Comment.'+this.form.assignment_id)
